...
public class BtreeBB<T extends Comparable<T>>  {
     ...
     * number of keys in a non-parent node.
     *
     * @param order
     *            of the B-Tree.
     */
    public BtreeBB(int order) {
    ...
    private void split(Node<T> nodeToSplit) {
        ...
        if (node.parent == null) {
            // new parent, height of tree is increased
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            if (node.parent != null && node.numberOfKeys() < minKeySize) {
                ...
                // Removing parent node with no keys or children
                root = null;
            }
        } else {
            // internal node
        ...
        if (rightNeighbor != null && rightNeighborSize > minKeySize) {
            ...
            if (leftNeighbor != null && leftNeighborSize > minKeySize) {
                ...
                if (parent.parent != null && parent.numberOfKeys() < minKeySize) {
                    ...
                    // parent no longer has keys, make this node the new parent
    ...
    private boolean validateNode(Node<T> node) {
        ...
        if (node.parent == null) {
            ...
            if (keySize > maxKeySize) {
                // check max key size. parent does not have a min key size
                return false;
            } else if (childrenSize == 0) {
                // if parent, no children, and keys are valid
                return true;
            } else if (childrenSize < 2) {
                // parent should have zero or at least two children
                return false;
